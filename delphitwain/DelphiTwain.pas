...
{10/11/2009 - Some changes to make it work in Delphi 2009, and above}

{01/11/2012 - Ondrej Pokorny: small changes for Lazarus and 64bit compiler}
...
{Units used bellow}
uses
  Controls{$IFDEF FPC}, interfacebase{$ENDIF};
...
begin
  ...
  begin
    ...
    begin
      ...
      //fHandle := Windows.LoadLibrary(PChar('TWAINDSM.dll'));
...
function TDelphiTwain.WndProc(var Message: TMessage): Boolean;
var
  i    : Integer;
  xMsg  : TMsg;
begin
...
begin
  ...
  if Assigned(Application.MainForm) and (Application.MainForm.Visible) then
    VirtualWindow := Application.MainFormHandle
  else
    VirtualWindow := WidgetSet.AppHandle;
  {$ELSE}
  VirtualWindow := Application.Handle;//Application.Handle;
  {$ENDIF}
  ...
  xTwainList.Add(Self);
  if not Assigned(xAppWndCallback) then
    //xAppWndCallback := {Windows.WNDPROC}(SetWindowLongPtr(WidgetSet.AppHandle,GWL_WNDPROC,PtrInt(@AppWndCallback)));
    xAppWndCallback := {Windows.WNDPROC}Pointer(SetWindowLongPtr(VirtualWindow,GWL_WNDPROC,NativeInt(@AppWndCallback)));
...
begin
  ...
  DTNativeUInt(LinePtr) := DTNativeUInt(ImageInfo.bmBits) + LineLength * CurLine;
  Ptr := LinePtr;
  DataSize := 0;
    ...
    repeat
      ...
      begin
        ...
        begin
          ...
          DTNativeUInt(Xfer.Memory.TheMem) :=
            DTNativeUInt(Xfer.Memory.TheMem) + Readed;
          ...
          begin
            ...
            {Adjust pointers}
            DTNativeUInt(LinePtr) := DTNativeUInt(LinePtr) - LineLength;
...
begin
  ...
  DTNativeUInt(Data) := DTNativeUInt(Dib) + Dib^.bmiHeader.biSize +
...
begin
  ...
  //lpBits := PChar(DibInfo^.bmiColors);
